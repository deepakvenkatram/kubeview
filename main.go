...
func getDashboardMetrics(clientset *kubernetes.Clientset, metricsClientset *metrics.Clientset) tea.Cmd {
	return func() tea.Msg {
		...
		for _, n := range topNodesCPU {
			nodeCPUData = append(nodeCPUData, barchart.BarData{Label: n.Name, Values: []barchart.BarValue{{Value: float64(nodeMetricsMap[n.Name].Usage[v1.ResourceCPU].MilliValue())}}})
		}
		...
		for _, n := range topNodesMem {
			nodeMemoryData = append(nodeMemoryData, barchart.BarData{Label: n.Name, Values: []barchart.BarValue{{Value: float64(nodeMetricsMap[n.Name].Usage[v1.ResourceMemory].Value() / (1024 * 1024))}}})
		}
